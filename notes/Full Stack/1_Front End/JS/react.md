...
# Fragments
## What is a Fragment?
- In react, you only return a single Element in the createElement() and render() functions.
- If you want to return multiple elements, you must wrap it in a parent Element!
- to get around this, use <React.Fragment> component.

## What are Fragments generally used for?
Fragments are for populating elements into a list or set, where the parent already exists.
It's good methods that return elements to the render(), used in loops and list functions.

## Syntax for Fragments?
- <>...</>
- <React.Fragment>...</React.Fragment>

## Do Fragments need a key?
- yes, fragments can take a key. Use it when working with lists.
...
## How should you use Higher Order Components?
Pass unrelated props through to the wrapped component using `...rest`.
It's okay to accept arguments after the wrappedComponent.
most common signature is: hocFunction(selector, actions)(wrappedComponent).
- In this case, hocFunction returns a function returning a HOC.
- mainly used for decoupling the HOC from the argument list; arguments are for configuring the type of HOC you want,
function is for returning a HOC.
- Since the function signature is always component -> component, it's really easy to compose together.
Wrap the display name for easy debugging: 
- there is a Component variable called myComponent.displayName that shows up in the React Developer Tools.
- add your HOC as `myHOCFunction(${WrappedComponent.displayName || WrappedComponent.name})

## How should you not use Higher Order Components?
Don't mutate the original Component and Component's prototype.
Don't use HOCs inside the render method
- it is treated as a new component every time; bad performance and you lose the old objects (states)!
Static Methods must be copied over
- WrappedComponent.staticMethod = function() {...}
- HOCComponent.staticMethod === 'undefined'
- so you need to copy the methods over (which you need to know exactly...)
- so best is to export the static methods separately from the component:
	- export default MyComponent;
	- export {someStaticFunction};
	- import MyComponent, { someFunction } from './myComponent.js';
- or use a third party to hoist non react statics: https://github.com/mridgway/hoist-non-react-statics
...
- you need to import React and the component to use.
- <MyComponent />
- Dot Notation: <MyComponent.subComponent />
- Must be capital!
- If you want to set the Type at runtime, use a variable to evaluate it:
	- const SpecifyComponent = components[props.componentName];
	- <SpecifyComponent />

## Props in JSX
- JS expressions in JSX: <MyComponent foo={1+2} />
- String Literal: <MyComponent foo='abc' />
- Props default to True: <MyComponent foo >
- spread using `...rest`
	- const props = {}
	- <MyComponent {...props} />
	- const { start, ...rest } = props;
	- <MyComponent start={start} {...rest} />

## Children in JSX
### String Literals
### JSX Children
### JS Expressions and Functions as Children
## unrendered, ignored children: False, null, undefined, true
